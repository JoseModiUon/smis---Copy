...
class MultipleModel extends Model
{
     ...
     * @param string $pk
     * @return array
     * @example  $desiredMultiple = Model::createMultiple(DesiredMultipleModel::className(), Yii::$app->request->post('DesiredMultipleModel'));
     * // Be specific, you may have multiple model classes in view
    ...
    protected static function processModels($post, $pk, $modelClass, array $multipleModels = []): array
    {
        ...
        if ($post && is_array($post)) {
            foreach ($post as $i => $item) {
                if (!empty($item[$pk]) && isset($multipleModels[$item[$pk]])) {
