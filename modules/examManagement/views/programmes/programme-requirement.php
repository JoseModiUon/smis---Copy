...
echo Html::a(
    ...
    ['/exam-management/programmes/programmes',
...
if(count($lvl_requirement) > 0) {
    ...
    foreach ($lvl_requirement as $Lvlrow) {
        ...
        if (count($lvlgrp) > 0) {
            ...
            echo '<tr class="groupBg"><td>Course Grouping</td> <td>Min Courses Taken</td><td>Min Pass Courses</td><td></td><td></td></tr>';
            foreach ($lvlgrp as $lvlgrprow) {
                echo '<tr>
                    ...
                    .Html::a('<i class="fa-solid fa-pen"></i> Edit', ['/exam-management/prog-curr-group-req/update',
                        ...
                        'prog_curriculum_id' => $_REQUEST['prog_curriculum_id'],'prog_study_level' => $Lvlrow['prog_study_level'], 'pcgrcid' => $lvlgrprow['prog_curr_group_requirement_id']], ['class' => 'btn btn-outline-success btn-sm']).'</td>
                            <td>   <a href="#" class="btn btn-outline-danger" text-align: center;"> <i class="fa fa-trash"></i> Remove</a></td>
                        </tr>';
