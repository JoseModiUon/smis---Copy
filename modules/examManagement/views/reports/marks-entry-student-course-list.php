...
$stu = Student::findOne(Yii::$app->request->queryParams['student_id']);


$this->title = 'Student Marks Entry';
$this->params['breadcrumbs'][] = ['label' => 'Exam Management', 'url' => ['/exam-management']];
$this->params['breadcrumbs'][] = $this->title;


$getResult = function ($marksheet_id) use ($stu) {
    $data = explode('_', $marksheet_id);
    $year = $data[0];
    $course_id = array_reverse($data)[1] . '_' . array_reverse($data)[0];
    $id = $stu->student_number . '-' . $year . '-' . $course_id;
    return ExStudentCourses::find()->where(['course_registration_id' => $id])->one();
};

$getExamType = function ($marksheet_id) use ($getResult) {

    $res = $getResult($marksheet_id);
            ...
            echo GridView::widget([
                ...
                'columns' => [
                    ...
                    [
                        'attribute' => 'exam_mark',
                        'label' => 'Exam Type',
                        'value' => function ($model) use ($getExamType) {
                            return $getExamType($model['mrksheet_id']);
                        }
                    ],
                    [
                        ...
                        'format' => 'raw',
                        'value' => function ($model) {
                            $id = $model['prog_curriculum_course_id'];
                            $marksheet_id = $model['marksheet_id'];
                            $timetable_id = $model['timetable_id'];
                            $course_work_mark = $model['course_work_mark'];
                            return '<input placeholder="0" id="' . $id . '_course_work_mark"  name="course_work_mark[]" type="text" class="form-control" marksheet="' . $marksheet_id . '" timetable="' . $timetable_id . '"  value="' . $course_work_mark . '" onchange="js:handleCourseWorkInput(this,\'' . $id . '\')"/>';
                        }
                    ],
                    [
                        'attribute' => 'exam_mark',
                        ...
                        'value' => function ($model) {
                            ...
                            return '<input placeholder="0" name="exam_mark[]" id="' . $id . '_exam_mark" type="text" class="form-control" marksheet="' . $marksheet_id . '" timetable="' . $timetable_id . '"   value="' . $exam_mark . '" onchange="js:handleExamMarkInput(this,\'' . $id . '\')"/>';
                        }
                    ],
                    [
                        ...
                        'value' => function ($model) {
                            ...
                            return '<input name="final_mark[]"  id="' . $id . '_final_mark" type="text" class="form-control"  value="' . $final_mark . '" readonly/>';
                        }
                    ],
                    [
                        'attribute' => 'Grade',
                        'format' => 'raw',
                        'value' => function ($model) use ($stu) {
                            $id = $model['prog_curriculum_course_id'];

                            $final_mark = $model['final_mark'];
                            $grade = getGrade($stu->student_number, $final_mark);
                            // return "<input  id={$reg_no}_grade type=text class=form-control readonly/ >";
                            // return '<input  id="' . $reg_no . '_grade" type="text" class="form-control"  readonly"/>';
                            return '<input id="' . $id . '_grade" value="' . $grade . '" type="text" class="form-control"  readonly />';
                        }
                    ],
                    [
                        'label' => 'Result Status',
                        ...
                        'value' => function ($model) use ($getResult) {
                            $res = $getResult($model['mrksheet_id']);
...
$urlGetExcelFile = Url::to(['/courseRegistration/reports/get-course-excel-file']);
$urlParseExcel = Url::to(['/courseRegistration/reports/parse-course-excel']);
...
$registration_number = $stu->student_number;
...
const parseExcel = async (file) => {
    ...
    formData.append("reg_no", registration_number)
...
const handleCourseWorkInput = (e, id)  => {
    let course_work_mark = e.value
    let exam_mark_id = id + "_exam_mark";
    let final_mark_id = id + "_final_mark";
    let grade_id = id + "_grade";
    const inputValues = {reg_no:registration_number,final_mark_id:final_mark_id,grade_id:grade_id}
...
const handleExamMarkInput = (e, id)  => {
    let exam_mark = e.value
    let course_work_id = id + "_course_work_mark";
    let final_mark_id = id + "_final_mark";
    let grade_id = id + "_grade";
    const inputValues = {reg_no:registration_number,final_mark_id:final_mark_id,grade_id:grade_id}
...
const saveMarks = (e) => {
    ...
    for (let input of course_work_mark_list) {
        let reg_no = registration_number
        ...
        const obj ={}
        obj[reg_no] = {course_work_mark:mark, marksheet_id:marksheet, timetable_id:timetable}
        // console.log(obj)

        if(inputValues.length > 0) {
            const index = inputValues.findIndex((item) => {
                return obj.reg_no == item.reg_no && obj.timetable_id == item.timetable_id && obj.marksheet_id == item.marksheet_id;
            })
            if(index > -1) {
                const newObj = {...inputValues.at(index)[reg_no], course_work_mark:mark}
                inputValues.splice(index,1,newObj)
            } else {
    ...
    for (let input of exam_mark_list) {
        ...
        const obj ={}
        obj[reg_no] = {exam_mark:mark, marksheet_id:marksheet, timetable_id:timetable}
        if(inputValues.length > 0) {
            const index = inputValues.findIndex((item) => {
                return obj.reg_no == item.reg_no && obj.timetable_id == item.timetable_id && obj.marksheet_id == item.marksheet_id;
            })
            if(index > -1) {
                const newObj = {}
                newObj[reg_no] = {...inputValues.at(index)[reg_no], exam_mark:mark}
                inputValues.splice(index,1,newObj)
            } else {
                inputValues.push(obj)
            }
...
const getFile = (data) => {
    $.ajax({
        ...
        success: function(resp) {
            ...
            link.href = url;
            link.download = registration_number +'.xlsx';
            link.click();

        },
        error: function(err) {
            // $('#app-is-loading-modal').modal('hide');
            krajeeDialog.alert('Creating excel failed.');
        }
    })
}
