...
class CourseController extends Controller
{
    ...
    public function actionSync()
    {
        ...
        SmisHelper::logMessage('Program Curriculum Course started.', __METHOD__);
        try {

            $auto = new AutoSynchronize();
            $auto->bulkSync(OrgProgCurrCourse::class);
            $admin->commit();
            $portal->commit();
            SmisHelper::logMessage('Program Curriculum Course sync finished.', __METHOD__);
        } catch (Exception $ex) {
            $portal->rollBack();
            $admin->rollBack();
