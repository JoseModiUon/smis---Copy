...
$this->params['breadcrumbs'][] = ['label' => 'Program Curriculum Courses', 'url' => ['/courseRegistration/org-prog-curr-course', 'OrgProgCurrCourseSearch[prog_curriculum_id]' => $session->get('prog_curriculum_id')]];
...
<?php
$urlSaveCourses = Url::to(['/courseRegistration/org-prog-curr-course/save-courses']);
$encodedIds = base64_encode(json_encode($assignedIds));

$coursesJs = <<< JS
let urlSaveCourses = '$urlSaveCourses';
let prog_curriculum_id = '$prog_curriculum_id'
let assignedIds = JSON.parse(atob("$encodedIds"))
...
$('#add-courses-grid-pjax').on('click', '#save-courses-btn', function(e){
    let selectedCoursesIds = assignedIds;
...
JS;
$this->registerJs($coursesJs, yii\web\View::POS_END);
